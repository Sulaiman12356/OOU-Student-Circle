...
export const AdminLoginPage: React.FC<AdminLoginPageProps> = ({ onNavigate }) => {
  const { loginAdmin, bootstrapSuperAdmin, isAdminAuthenticated } = useAdminAuth();
        ...
        <div className="max-w-md w-full bg-[#061A4F] p-8 rounded-3xl border border-[#F5B400]/40 shadow-2xl text-center space-y-6">
          ...
          <div className="space-y-2">
            ...
            <p className="text-xs text-slate-300">
              You are currently authenticated with administrative privileges.
            </p>
          </div>
          <button
            onClick={() => onNavigate('/admin')}
            className="w-full py-3.5 bg-[#F5B400] hover:bg-[#e0a400] text-[#061A4F] font-black rounded-xl transition flex items-center justify-center gap-2 shadow-lg"
          >
  ...
  const handleBootstrapSuperAdmin = async () => {
    ...
    setIsLoading(false);
    if (res.success) {
      setSuccessMsg('Super Administrator initialized successfully! Redirecting...');
      setTimeout(() => {
        onNavigate('/admin');
      }, 800);
    } else {
  ...
  return (
    <div className="min-h-screen bg-[#040E29] flex flex-col justify-center py-12 sm:px-6 lg:px-8 relative overflow-hidden">
      ...
      <div className="sm:mx-auto sm:w-full sm:max-w-md relative z-10 space-y-6">
        ...
        <button
          onClick={() => onNavigate('/')}
          className="inline-flex items-center gap-2 text-xs font-semibold text-slate-400 hover:text-white transition px-3 py-1.5 rounded-lg bg-white/5 border border-white/10"
        >
        ...
        <div className="text-center space-y-3">
          ...
          <h1 className="text-2xl sm:text-3xl font-black text-white tracking-tight">
            OOU StudentCircle Admin
          </h1>
          <p className="text-xs text-slate-400 max-w-sm mx-auto">
            Authorized personnel portal for platform governance, identity verification, and financial arbitration.
          </p>
        </div>
        ...
        <div className="bg-[#061A4F]/90 backdrop-blur-xl py-8 px-6 sm:px-10 rounded-3xl border border-[#F5B400]/30 shadow-2xl space-y-6">
            ...
            <div className="space-y-1.5">
              <div className="flex items-center justify-between">
                ...
                <button
                  ...
                  className="text-[11px] font-semibold text-[#F5B400] hover:underline"
                >
                  Forgot Password?
                </button>
              </div>
            ...
            <div className="flex items-center justify-between pt-1">
              <label className="flex items-center gap-2 cursor-pointer">
                <input
                  ...
                  className="w-3.5 h-3.5 text-[#F5B400] rounded bg-[#040E29] border-slate-700 focus:ring-0"
                />
                <span className="text-xs text-slate-300">Remember administrative session</span>
              </label>
            </div>
          ...
          {/* Platform Owner First-Run Setup / Bootstrap helper */}
          <div className="pt-4 border-t border-slate-700/50 space-y-3">
            <div className="flex items-center justify-between text-[11px] text-slate-400">
              <span className="flex items-center gap-1.5">
                <Server className="w-3 h-3 text-[#F5B400]" />
                <span>Platform Owner Initializer</span>
              </span>
            ...
            >
              ...
              <span>Bootstrap Super Admin ({SUPER_ADMIN_EMAIL})</span>
            </button>
          </div>
        ...
        <div className="text-center text-[11px] text-slate-500 space-y-1">
          ...
          <p>All administrative logins, queries, and mutations are immutably audited.</p>
        </div>
          ...
          <div className="bg-[#061A4F] border border-[#F5B400]/40 max-w-md w-full p-6 rounded-3xl shadow-2xl space-y-4 text-white">
            ...
            {forgotSuccess ? (
              <div className="space-y-4">
                ...
                <button
                  onClick={() => setShowForgotModal(false)}
                  className="w-full py-2.5 bg-[#F5B400] text-[#061A4F] font-bold text-xs rounded-xl"
                >
                  Return to Login
                </button>
              </div>
              ...
              <form onSubmit={handlePasswordReset} className="space-y-4 text-xs">
                ...
                <div className="flex gap-2 justify-end pt-2">
                  ...
                  <button
                    ...
                    className="px-4 py-2 bg-[#F5B400] hover:bg-[#e0a400] text-[#061A4F] rounded-xl text-xs font-bold"
                  >
